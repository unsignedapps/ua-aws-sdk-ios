//
//  UASNSPublishRequest.m
//  AWS iOS SDK
//
//  Copyright © Unsigned Apps 2014. See License file.
//  Created by Rob Amos.
//
//

#import "UASNSPublishRequest.h"
#import "UASNSPublishResponse.h"

@interface UASNSPublishRequest ()

@property (nonatomic, copy) NSString *action;
@property (nonatomic, copy) NSString *version;

@end

@implementation UASNSPublishRequest

@synthesize action=_action, version=_version, topicARN=_topicARN, targetARN=_targetARN, message=_message, subject=_subject, messageStructure=_messageStructure;

- (id)init
{
	if (self = [super init])
	{
		[self setAction:@"Publish"];
		[self setVersion:@"2010-03-31"];
	}
	return self;
}

- (Class)UA_ResponseClass
{
	return [UASNSPublishResponse class];
}

+ (NSDictionary *)queryStringKeyPathsByPropertyKey
{
    // Start with super's key paths (if there are any)
    NSMutableDictionary *keyPaths = [[UASNSRequest queryStringKeyPathsByPropertyKey] mutableCopy];

    [keyPaths addEntriesFromDictionary:
    @{
        @"action": @"Action",
        @"version": @"Version",
        @"topicARN": @"TopicArn",
        @"targetARN": @"TargetArn",
        @"message": @"Message",
        @"subject": @"Subject",
        @"messageStructure": @"MessageStructure"
    }];
    return [keyPaths copy];
}

- (void)setAction:(NSString *)action
{
	_action = action;
	
	if (![self.UA_dirtyProperties containsObject:@"action"])
		[self.UA_dirtyProperties addObject:@"action"];
}

- (void)setVersion:(NSString *)version
{
	_version = version;
	
	if (![self.UA_dirtyProperties containsObject:@"version"])
		[self.UA_dirtyProperties addObject:@"version"];
}

- (void)setTopicARN:(NSString *)topicARN
{
	_topicARN = topicARN;
	
	if (![self.UA_dirtyProperties containsObject:@"topicARN"])
		[self.UA_dirtyProperties addObject:@"topicARN"];
}

- (void)setTargetARN:(NSString *)targetARN
{
	_targetARN = targetARN;
	
	if (![self.UA_dirtyProperties containsObject:@"targetARN"])
		[self.UA_dirtyProperties addObject:@"targetARN"];
}

- (void)setMessage:(NSString *)message
{
	_message = message;
	
	if (![self.UA_dirtyProperties containsObject:@"message"])
		[self.UA_dirtyProperties addObject:@"message"];
}

- (void)setSubject:(NSString *)subject
{
	_subject = subject;
	
	if (![self.UA_dirtyProperties containsObject:@"subject"])
		[self.UA_dirtyProperties addObject:@"subject"];
}

- (void)setMessageStructure:(NSString *)messageStructure
{
	_messageStructure = messageStructure;
	
	if (![self.UA_dirtyProperties containsObject:@"messageStructure"])
		[self.UA_dirtyProperties addObject:@"messageStructure"];
}

#pragma mark - Invocation

- (void)invokeWithOwner:(id)owner completionBlock:(UASNSPublishRequestCompletionBlock)completionBlock
{
    [self setUA_Owner:owner];
    [self setUA_RequestCompletionBlock:completionBlock];
    [self invoke];
}

- (void)waitWithOwner:(id)owner shouldContinueWaitingBlock:(UASNSPublishRequestShouldContinueWaitingBlock)shouldContinueWaitingBlock completionBlock:(UASNSPublishRequestCompletionBlock)completionBlock
{
    [self setUA_Owner:owner];
    [self setUA_ShouldContinueWaiting:shouldContinueWaitingBlock];
    [self setUA_RequestCompletionBlock:completionBlock];
    [self invoke];
}

- (void)waitWithOwner:(id)owner untilValueAtKeyPath:(NSString *)keyPath isInArray:(NSArray *)array completionBlock:(UASNSPublishRequestCompletionBlock)completionBlock
{
    [self setUA_Owner:self];
    [self setUA_ShouldContinueWaiting:[UAAWSRequest UA_ShouldContinueWaitingBlockUntilValueAtKeyPath:keyPath isInArray:array]];
    [self setUA_RequestCompletionBlock:completionBlock];
    [self invoke];
}

@end
