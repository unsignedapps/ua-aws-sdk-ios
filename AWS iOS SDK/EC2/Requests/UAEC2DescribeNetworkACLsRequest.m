...
{
    if (self.networkACLIDs == nil || index >= ([self.networkACLIDs count]-1))
        return nil;

    return [self.networkACLIDs objectAtIndex:index];
}
...
- (void)setNetworkACLIDs:(NSMutableArray *)networkACLIDs
{
	_networkACLIDs = networkACLIDs;
	
	if (![self.UA_dirtyProperties containsObject:@"networkACLIDs"])
		[self.UA_dirtyProperties addObject:@"networkACLIDs"];
}
