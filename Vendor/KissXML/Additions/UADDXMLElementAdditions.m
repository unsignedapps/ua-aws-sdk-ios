#import "UADDXMLElementAdditions.h"

@implementation UADDXMLElement (UADDAUADDitions)
...
{
	UADDXMLElement *element = [UADDXMLElement elementWithName:name];
	[element setXmlns:ns];
	return element;
}
...
- (void)setXmlns:(NSString *)ns
{
	...
	[self aUADDNamespace:[UADDXMLNode namespaceWithName:@"" stringValue:ns]];
}
...
{
	return [self XMLStringWithOptions:(UADDXMLNodePrettyPrint | UADDXMLNodeCompactEmptyElement)];
}
...
{
    return [self XMLStringWithOptions:UADDXMLNodeCompactEmptyElement];
}

/**
 *	Shortcut to avoid having to manually create a UADDXMLNode everytime.
**/
- (void)aUADDAttributeWithName:(NSString *)name stringValue:(NSString *)string
{
	[self aUADDAttribute:[UADDXMLNode attributeWithName:name stringValue:string]];
}
...
- (NSDictionary *)attributesAsDictionary
{
	...
	{
		UADDXMLNode *node = [attributes objectAtIndex:i];
