...
static void MarkBirth(void *xmlPtr, UADDXMLNode *wrapper);
static void MarkDeath(void *xmlPtr, UADDXMLNode *wrapper);
...
+ (void)initialize
{
	...
	dispatch_once(&onceToken, ^{
		...
		#if UADDXML_DEBUG_MEMORY_ISSUES
		{
			...
			zombieQueue = dispatch_queue_create("UADDXMLZombieQueue", NULL);
		}
		#endif
...
{
	return [[UADDXMLElement alloc] initWithName:name];
}

+ (id)elementWithName:(NSString *)name stringValue:(NSString *)string
{
	return [[UADDXMLElement alloc] initWithName:name stringValue:string];
}

+ (id)elementWithName:(NSString *)name children:(NSArray *)children attributes:(NSArray *)attributes
{
	UADDXMLElement *result = [[UADDXMLElement alloc] initWithName:name];
...
{
	return [[UADDXMLElement alloc] initWithName:name URI:URI];
}
...
+ (id)attributeWithName:(NSString *)name stringValue:(NSString *)stringValue
{
	...
	return [[UADDXMLAttributeNode alloc] initWithAttrPrimitive:attr owner:nil];
}
...
+ (id)attributeWithName:(NSString *)name URI:(NSString *)URI stringValue:(NSString *)stringValue
{
	...
	UADDXMLAttributeNode *result = [[UADDXMLAttributeNode alloc] initWithAttrPrimitive:attr owner:nil];
	[result setURI:URI];
...
+ (id)namespaceWithName:(NSString *)name stringValue:(NSString *)stringValue
{
	...
	return [[UADDXMLNamespaceNode alloc] initWithNsPrimitive:ns nsParent:NULL owner:nil];
}
...
+ (id)processingInstructionWithName:(NSString *)name stringValue:(NSString *)stringValue
{
	...
	return [[UADDXMLNode alloc] initWithPrimitive:(xmlKindPtr)procInst owner:nil];
}
...
+ (id)commentWithStringValue:(NSString *)stringValue
{
	...
	return [[UADDXMLNode alloc] initWithPrimitive:(xmlKindPtr)comment owner:nil];
}
...
+ (id)textWithStringValue:(NSString *)stringValue
{
	...
	return [[UADDXMLNode alloc] initWithPrimitive:(xmlKindPtr)text owner:nil];
}
...
+ (id)nodeWithUnknownPrimitive:(xmlKindPtr)kindPtr owner:(UADDXMLNode *)owner
{
	if (kindPtr->type == XML_DOCUMENT_NODE)
	{
		return [UADDXMLDocument nodeWithDocPrimitive:(xmlDocPtr)kindPtr owner:owner];
	}
	else if (kindPtr->type == XML_ELEMENT_NODE)
	{
		return [UADDXMLElement nodeWithElementPrimitive:(xmlNodePtr)kindPtr owner:owner];
	}
	else if (kindPtr->type == XML_NAMESPACE_DECL)
	{
		// Todo: This may be a problem...
		
		return [UADDXMLNamespaceNode nodeWithNsPrimitive:(xmlNsPtr)kindPtr nsParent:NULL owner:owner];
	}
	else if (kindPtr->type == XML_ATTRIBUTE_NODE)
	{
		return [UADDXMLAttributeNode nodeWithAttrPrimitive:(xmlAttrPtr)kindPtr owner:owner];
	}
	else
	{
		return [UADDXMLNode nodeWithPrimitive:kindPtr owner:owner];
	}
}
...
{
	return [[UADDXMLNode alloc] initWithPrimitive:kindPtr owner:owner];
}
...
- (id)init
{
	...
	if ([self isKindOfClass:[UADDXMLInvalidNode class]])
	{
		return self;
	}
	else
	{
		return [[UADDXMLInvalidNode alloc] init];
	}
}
...
- (void)dealloc
{
	...
	// This may be the case if, e.g., UADDXMLElement calls [self release] from it's init method.
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	if (IsXmlDocPtr(genericPtr))
	{
		...
		return [[UADDXMLDocument alloc] initWithDocPrimitive:copyDocPtr owner:nil];
	}
	...
	if (IsXmlNodePtr(genericPtr))
	{
		...
		if ([self isKindOfClass:[UADDXMLElement class]])
			return [[UADDXMLElement alloc] initWithElementPrimitive:copyNodePtr owner:nil];
		else
			return [[UADDXMLNode alloc] initWithPrimitive:(xmlKindPtr)copyNodePtr owner:nil];
	}
	...
	if (IsXmlAttrPtr(genericPtr))
	{
		...
		return [[UADDXMLAttributeNode alloc] initWithAttrPrimitive:copyAttrPtr owner:nil];
	}
	...
	if (IsXmlNsPtr(genericPtr))
	{
		...
		return [[UADDXMLNamespaceNode alloc] initWithNsPrimitive:copyNsPtr nsParent:NULL owner:nil];
	}
	...
	if (IsXmlDtdPtr(genericPtr))
	{
		...
		return [[UADDXMLNode alloc] initWithPrimitive:(xmlKindPtr)copyDtdPtr owner:nil];
	}
...
{
	// UADDXMLNode, UADDXMLElement, and UADDXMLDocument are simply light-weight wrappers atop a libxml structure.
	// 
	...
	{
		UADDXMLNode *aNode = (UADDXMLNode *)anObject;
...
/**
 * Returns the UADDXMLDocument object containing the root element and representing the XML document as a whole.
...
- (UADDXMLDocument *)rootDocument
{
	...
	if (node == NULL || node->doc == NULL)
		...
		return [UADDXMLDocument nodeWithDocPrimitive:node->doc owner:self];
}
...
- (UADDXMLNode *)parent
{
	...
	if (node->parent == NULL)
		...
		return [UADDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)node->parent owner:self];
}
...
/**
 * Returns an immutable array containing the child nodes of the receiver (as UADDXMLNode objects).
**/
- (NSArray *)children
{
	// Note: DDXMLNamespaceNode overrides this method
	...
	while (child != NULL)
	{
		[result addObject:[UADDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)child owner:self]];
 ...
 * Returns a UADDXMLNode object or nil if the receiver has no children.
 * 
 * If the receive has children and index is out of bounds, an exception is raised.
 * 
 * The receiver should be a UADDXMLNode object representing a document, element, or document type declaration.
...
- (UADDXMLNode *)childAtIndex:(NSUInteger)index
{
	...
	while (child != NULL)
	{
		if (i == index)
		{
			return [UADDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)child owner:self];
		}
...
/**
 * Returns the previous UADDXMLNode object that is a sibling node to the receiver.
 * 
...
- (UADDXMLNode *)previousSibling
{
	...
	if (node->prev == NULL)
		...
		return [UADDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)node->prev owner:self];
}

/**
 * Returns the next UADDXMLNode object that is a sibling node to the receiver.
 * 
...
- (UADDXMLNode *)nextSibling
{
	...
	if (node->next == NULL)
		...
		return [UADDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)node->next owner:self];
}

/**
 * Returns the previous UADDXMLNode object in document order.
 * 
 ...
 * nil is returned. UADDXMLNode bypasses namespace and attribute nodes when it traverses a tree in document order.
**/
- (UADDXMLNode *)previousNode
{
	...
	if (previousSibling != NULL)
	{
		if (previousSibling->last != NULL)
		{
			...
			return [UADDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)lastChild owner:self];
		}
		else
		{
			// The previous sibling has no children, so the previous node is simply the previous sibling
			return [UADDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)previousSibling owner:self];
		}
	}
...
/**
 * Returns the next UADDXMLNode object in document order.
 * 
 ...
 * UADDXMLNode bypasses namespace and attribute nodes when it traverses a tree in document order.
**/
- (UADDXMLNode *)nextNode
{
	...
	UADDXMLNode *firstChild = [self childAtIndex:0];
	if (firstChild)
		return firstChild;
	
	// If the node has a next sibling, then next node is the same as next sibling
	
	UADDXMLNode *nextSibling = [self nextSibling];
	if (nextSibling)
		return nextSibling;
	...
	while (parent != NULL)
	{
		...
		if (parentNextSibling != NULL)
			return [UADDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)parentNextSibling owner:self];
		else
 ...
 *
 * This method is applicable to UADDXMLNode objects representing elements, text, comments, processing instructions,
...
- (NSString *)XMLStringWithOptions:(NSUInteger)options
{
	...
	if (options & UADDXMLNodeCompactEmptyElement)
		xmlSaveNoEmptyTags = 0;
	else
		xmlSaveNoEmptyTags = 1;
	
	int format = 0;
	if (options & UADDXMLNodePrettyPrint)
	{
		format = 1;
	...
	else
	{
		...
		else
		{
			...
			for (i = 0; i < count; i++)
			{
				...
				[mResult addObject:[UADDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)node owner:self]];
			}
...
// 
// UADDXMLElement *root = [[UADDXMLElement alloc] initWithXMLString:str error:nil];
// NSArray *children = [root children];
// 
// dispatch_queue_t q = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
// dispatch_apply([children count], q, ^(size_t i) {
//     UADDXMLElement *child = [children objectAtIndex:i];
//     <process child>
// });
// 
// 
...
// 
// UADDXMLElement *root = [[UADDXMLElement alloc] initWithXMLString:str error:nil];
// UADDXMLElement *child = [root elementForName:@"starbucks"];
// 
 ...
 * Errors are caught in the MyErrorHandler method within UADDXMLDocument.
**/
+ (NSError *)lastError
{
	return [[[NSThread currentThread] threadDictionary] objectForKey:UADDLastErrorKey];
}
...
static void MyErrorHandler(void * userData, xmlErrorPtr error)
{
	...
	if (error == NULL)
	{
		[[[NSThread currentThread] threadDictionary] removeObjectForKey:UADDLastErrorKey];
	}
	else
	{
		...
		[[[NSThread currentThread] threadDictionary] setObject:errorObject forKey:UADDLastErrorKey];
	}
}
...
// and the values are mutable arrays. Any wrapper objects (UADDXMLElement, UADDXMLNode, etc) get added to the
...
BOOL UADDXMLIsZombie(void *xmlPtr, UADDXMLNode *wrapper)
{
	// This method only exists if DDXML_DEBUG_MEMORY_ISSUES is enabled.
...
@implementation UADDXMLNamespaceNode

/**
 * Returns a DDXML wrapper object for the given primitive node.
 * The given node MUST be non-NULL and of the proper type.
**/
+ (id)nodeWithNsPrimitive:(xmlNsPtr)ns nsParent:(xmlNodePtr)parent owner:(UADDXMLNode *)owner
{
	return [[UADDXMLNamespaceNode alloc] initWithNsPrimitive:ns nsParent:parent owner:owner];
}
 ...
 * The given node MUST be non-NULL and of the proper type.
**/
- (id)initWithNsPrimitive:(xmlNsPtr)ns nsParent:(xmlNodePtr)parent owner:(UADDXMLNode *)inOwner
{
...
- (UADDXMLNode *)parent
{
	...
	if (nsParentPtr == NULL)
		...
		return [UADDXMLNode nodeWithUnknownPrimitive:(xmlKindPtr)nsParentPtr owner:self];
}
...
- (void)detach
{
	...
	if (nsParentPtr != NULL)
	{
		[UADDXMLNode detachNamespace:(xmlNsPtr)genericPtr fromNode:nsParentPtr];
...
@implementation UADDXMLAttributeNode

+ (id)nodeWithAttrPrimitive:(xmlAttrPtr)attr owner:(UADDXMLNode *)owner
{
	return [[UADDXMLAttributeNode alloc] initWithAttrPrimitive:attr owner:owner];
}

- (id)initWithAttrPrimitive:(xmlAttrPtr)attr owner:(UADDXMLNode *)inOwner
{
...
- (void)setURI:(NSString *)URI
{
	...
	// There is no way, within the API, to get a UADDXMLNode wrapper for the attribute's namespace.
...
@implementation UADDXMLInvalidNode
