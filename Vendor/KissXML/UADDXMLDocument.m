...
@implementation UADDXMLDocument
...
{
	return [[UADDXMLDocument alloc] initWithDocPrimitive:doc owner:owner];
}

- (id)initWithDocPrimitive:(xmlDocPtr)doc owner:(UADDXMLNode *)inOwner
{
...
/**
 * Initializes and returns a UADDXMLDocument object created from an NSData object.
 * 
 * Returns an initialized UADDXMLDocument object, or nil if initialization fails
...
- (id)initWithData:(NSData *)data options:(NSUInteger)mask error:(NSError **)error
{
	...
	{
		if (error) *error = [NSError errorWithDomain:@"UADDXMLErrorDomain" code:0 userInfo:nil];
...
- (UADDXMLElement *)rootElement
{
	...
	if (rootNode != NULL)
		return [UADDXMLElement nodeWithElementPrimitive:rootNode owner:self];
	else
		return nil;
}
