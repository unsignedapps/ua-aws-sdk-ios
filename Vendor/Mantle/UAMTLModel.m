...
// This coupling is needed for backwards compatibility in UAMTLModel's deprecated
// methods.
#import "UAMTLJSONAdapter.h"
#import "UAMTLModel+NSCoding.h"

// Used to cache the reflection performed in +propertyKeys.
static void *UAMTLModelCachedPropertyKeysKey = &UAMTLModelCachedPropertyKeysKey;
...
+ (void)enumeratePropertiesUsingBlock:(void (^)(objc_property_t property, BOOL *stop))block {
	...
	while (!stop && ![cls isEqual:UAMTLModel.class]) {
		unsigned count = 0;
...
+ (NSSet *)propertyKeys {
	NSSet *cachedKeys = objc_getAssociatedObject(self, UAMTLModelCachedPropertyKeysKey);
	...
	objc_setAssociatedObject(self, UAMTLModelCachedPropertyKeysKey, keys, OBJC_ASSOCIATION_COPY);
...
- (void)mergeValueForKey:(NSString *)key fromModel:(UAMTLModel *)model {
...
- (void)mergeValuesForKeysFromModel:(UAMTLModel *)model {
