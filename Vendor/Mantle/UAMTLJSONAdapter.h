...
// A UAMTLModel object that supports being parsed from and serialized to JSON.
@protocol UAMTLJSONSerializing
@required
...
// back to JSON.
//
// If the receiver implements a `+<key>JSONTransformer` method, UAMTLJSONAdapter
...
// be passed into -[UAMTLJSONAdapter initWithJSONDictionary:modelClass:], but
...
// The domain for errors originating from UAMTLJSONAdapter.
extern NSString * const UAMTLJSONAdapterErrorDomain;

// +classForParsingJSONDictionary: returned nil for the given dictionary.
extern const NSInteger UAMTLJSONAdapterErrorNoClassFound;

// The provided JSONDictionary is not valid.
extern const NSInteger UAMTLJSONAdapterErrorInvalidJSONDictionary;

// Converts a UAMTLModel object to and from a JSON dictionary.
@interface UAMTLJSONAdapter : NSObject

// The model object that the receiver was initialized with, or that the receiver
// parsed from a JSON dictionary.
@property (nonatomic, strong, readonly) NSObject<UAMTLModel, UAMTLJSONSerializing> * model;

// Attempts to parse a JSON dictionary into a model object.
//
// modelClass     - The subclass to attempt to parse from the JSON.
//                  This class must conform to <UAMTLModel, UAMTLJSONSerializing>. This
...
//                  UAMTLJSONAdapterErrorInvalidJSONDictionary.
// modelClass     - The NSObject subclass to attempt to parse from the JSON.
//                  This class must conform to <UAMTLModel, UAMTLJSONSerializing>. This
//                  argument must not be nil.
// error          - If not NULL, this may be set to an error that occurs during
//                  parsing or initializing an instance of `modelClass`.
...
@interface UAMTLJSONAdapter (Deprecated)
