...
#define onExit \
    try {} @finally {} \
    __strong UAMTL_cleanupBlock_t metamacro_concat(UAMTL_exitBlock_, __LINE__) __attribute__((cleanup(UAMTL_executeCleanupBlock), unused)) = ^
...
#define weakify(...) \
    try {} @finally {} \
    metamacro_foreach_cxt(UAMTL_weakify_,, __weak, __VA_ARGS__)
...
#define unsafeify(...) \
    try {} @finally {} \
    metamacro_foreach_cxt(UAMTL_weakify_,, __unsafe_unretained, __VA_ARGS__)
...
#define strongify(...) \
    ...
    metamacro_foreach(UAMTL_strongify_,, __VA_ARGS__) \
    _Pragma("clang diagnostic pop")

/*** implementation details follow ***/
typedef void (^UAMTL_cleanupBlock_t)();

void UAMTL_executeCleanupBlock (__strong UAMTL_cleanupBlock_t *block);

#define UAMTL_weakify_(INDEX, CONTEXT, VAR) \
    CONTEXT __typeof__(VAR) metamacro_concat(VAR, _weak_) = (VAR);

#define UAMTL_strongify_(INDEX, VAR) \
