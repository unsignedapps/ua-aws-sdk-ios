...
UAMTL_propertyAttributes *UAMTL_copyPropertyAttributes (objc_property_t property) {
    ...
    UAMTL_propertyAttributes *attributes = calloc(1, sizeof(UAMTL_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate UAMTL_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        case 'C':
            attributes->memoryManagementPolicy = UAMTL_propertyMemoryManagementPolicyCopy;
            break;

        case '&':
            attributes->memoryManagementPolicy = UAMTL_propertyMemoryManagementPolicyRetain;
            break;
